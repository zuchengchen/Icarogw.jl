...
`reference_catalog_kcorr.csv` and `reference_catalog_em.csv` capture
dependency-light formulas from `catalog.py`: modern and deprecated
K-corrections plus the EM redshift likelihood-prior helper. They are generated
by `--suite catalog-formulas-smoke`, which stubs optional catalog file and
HEALPix imports.
