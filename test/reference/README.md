...
`Bivariate2DGaussian`, dependency-free paired and multi-peak mass wrapper smoke
cases, and the `CBC_mixte_pop_rate` logaddexp mixture identity. It is generated
by `--suite priors-rates-smoke`. Python's paired mass wrapper normalization
uses a fixed-seed Monte Carlo estimate, so those comparisons allow a small
absolute tolerance.
